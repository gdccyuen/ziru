...
CREATE TABLE "sources" (
	...
	"failure_reason" text,
	"knowhere_job_id" text,
	"knowhere_document_id" text,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL,
	"deleted_at" timestamp with time zone
);
...
CREATE INDEX "chat_threads_workspace_updated_idx" ON "chat_threads" USING btree ("workspace_id","updated_at" DESC NULLS LAST) WHERE deleted_at IS NULL;--> statement-breakpoint
CREATE INDEX "sources_workspace_created_idx" ON "sources" USING btree ("workspace_id","created_at" DESC NULLS LAST) WHERE deleted_at IS NULL;--> statement-breakpoint
