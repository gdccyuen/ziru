...
The WebUI shipped with a demo catalog system that served static content to
...
- **Domain layer:** `src/domains/demo/`, `src/integrations/ziru-demo.ts`,
...
Library. All real documents come from Ziru namespaces. Guest mode provided
...
1. **Delete** all demo-specific files: `src/integrations/ziru-demo.ts`,
...
4. **Remove guest mode:** the proxy no longer allows anonymous source reads.
   ...
   `webuiRequestContext`. Unauthenticated SSR returns `{ sources: [] }`.
...
6. **Replace the Official Library panel with a namespace dropdown** in the
   ...
   Ziru's `GET /v1/documents/namespaces`) and lets users import all
...
7. **Eagerly localize compatible-namespace documents** on every source list
   ...
   pre-filters against existing DB rows by `ziruDocumentId` so only
   genuinely new documents are upserted.

## Consequences

- **Simpler codebase:** ~6000 lines removed across 84 files.
- **No anonymous access:** self-hosted deployments require `ZIRU_API_KEY`
...
- **All sources are real:** no static/demo content. Sources are either
  ...
  (transient Ziru documents not yet localized).
- **Eager localization means new Ziru documents appear automatically:**
...
- **Namespace dropdown extends beyond compatible namespaces:** users can
  import from any Ziru namespace, not just `default` and the workspace
