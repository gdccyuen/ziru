...
- **Search (Q18, revised after PM review):** POST /v2/search — structured JSON; `query` REQUIRED non-empty (empty query rejected; no browse mode inside the search engine — engine stays frozen per Q5); `filters` MANDATORY bag of attribute criteria (never an unscoped call); engine parameters keep today's contract (topK, internalRecallK, rerank, agentic). Attribute browsing is `GET /v2/documents` with the same filter syntax (profile-scoped, newest first).
