...
class GuestRegisterRequest(BaseModel):
    ...
    device_id: str = Field(
        ..., min_length=1, max_length=255, description="Stable client device identifier"
    )
    client: str = Field(
        ...,
        min_length=1,
    ...
    app_version: str | None = Field(
        default=None, max_length=32, description="Client application version"
    )
...
class GuestRegisterResponse(BaseModel):
    ...
    device_id: str
    api_key: str = Field(
        description="Plaintext API key (sk_...), issued on first registration"
    )
    rate_limit: GuestRateLimitInfo
    expires_at: datetime | None = Field(
        description="API key expiry timestamp, or null for non-expiring guest keys"
    )
