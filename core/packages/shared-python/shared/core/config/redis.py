...
class RedisConfig(BaseModel):
    ...
    REDIS_SYNC_MAX_CONNECTIONS: int = Field(
        default=50, description="Worker sync pool max connections"
    )
    REDIS_SYNC_POOL_TIMEOUT: int = Field(
        default=5, description="Worker sync pool blocking timeout (seconds)"
    )
    ...
    REDIS_HEALTH_CHECK_TIMEOUT: float = Field(
...
class RedisPoolManager:
    ...
    async def init_pool(self):
        ...
        async with self._lock:
            if self._redis_service is None:
                ...
                logger.warning(
                    "The shared Redis service already exists; skipping initialization."
                )
    ...
    async def get_pool(self):
        ...
        if self._redis_service is None:
            logger.error(
                "RedisPoolManager.get_pool() was called before the service was initialized"
            )
